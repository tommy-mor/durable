...
pub fn bind(lua: &Lua, data_dir: &Path) -> mlua::Result<bool> {
    ...
    // threads), but this reducer wraps a Lua VM that never leaves this
    // thread. The wrapper asserts that: the Runtime it goes into is owned
    // by an Rc below, so it cannot cross threads either.
...

    {
        ...
        store.set(
            ...
            lua.create_function(move |lua, event: LuaValue| {
                ...
                *lua_slot.borrow_mut() = Some(lua as *const Lua);
                let rec = rt.borrow_mut().append(ev).map_err(mlua::Error::external)?;
                *lua_slot.borrow_mut() = None;
                Ok(rec.seq)
            })?,
        )?;
